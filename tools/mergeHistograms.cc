...
Options ParseArgs(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--input") {
            ...
            std::cout << "Usage:\n"
                      << "  mergeHistograms --input output/run1 --pattern hist_edep_raw_t*.csv --output merged.csv\n"
...
std::vector<Bin> ReadHistogram(const fs::path& file)
{
    ...
    if (!input) throw std::runtime_error("Cannot open input file: " + file.string());
    ...
    if (bins.empty()) {
        throw std::runtime_error("No histogram bins found in file: " + file.string());
    }
    return bins;
}
...
void MergeAndWrite(const std::vector<fs::path>& files, const fs::path& output)
{
    ...
    for (std::size_t fileIndex = 0; fileIndex < files.size(); ++fileIndex) {
        ...
        if (bins.size() != merged.size()) {
            throw std::runtime_error("Bin count mismatch in file: " + files[fileIndex].string());
        }
        for (std::size_t i = 0; i < bins.size(); ++i) {
            if (bins[i].low != merged[i].low || bins[i].high != merged[i].high) {
                throw std::runtime_error("Bin edge mismatch in file: " + files[fileIndex].string() +
